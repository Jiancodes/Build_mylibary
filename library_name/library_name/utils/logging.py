...
from .configuration import proto_sep_config
...
library_name_dev_log_handler = handlers.TimedRotatingFileHandler(
...
library_name_dev_log_handler.setFormatter(_dev_formatter)
library_name_dev_log_handler.setLevel(logging.DEBUG)
# now set up the usr log which will save the info

library_name_usr_log_handler = handlers.TimedRotatingFileHandler(
    get_path_of_log_file("usr.log"), when="D", interval=1, backupCount=10
)

library_name_usr_log_handler.setLevel(logging.INFO)
...
library_name_usr_log_handler.setFormatter(_usr_formatter)
...
library_name_console_log_handler = RichHandler(
    level=proto_sep_config.logging.level, rich_tracebacks=True, markup=True, console=console
)
library_name_console_log_handler.setFormatter(_console_formatter)
...
def silence_warnings():
    ...
    library_name_usr_log_handler.addFilter(warning_filter)
    library_name_console_log_handler.addFilter(warning_filter)
...
def activate_warnings():
    ...
    library_name_usr_log_handler.removeFilter(warning_filter)
    library_name_console_log_handler.removeFilter(warning_filter)


def update_logging_level(level):

    library_name_console_log_handler.setLevel(level)
...
def setup_logger(name):
    ...
    log.addHandler(library_name_dev_log_handler)

    log.addHandler(library_name_console_log_handler)

    log.addHandler(library_name_usr_log_handler)
